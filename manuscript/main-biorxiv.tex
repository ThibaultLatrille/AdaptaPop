...
Empirically, phenomenological and mechanistic {codon} methods should be confronted to McDonald \& Kreitman ({MK}) methods.
In the case of the overlap between positively selected genes detected with phenomenological {codon} models and with {MK} methods, the set of genes detected does not seem to overlap beyond random expectations~\cite{He2020}.
In contrast, at the site level, classical {codon} models are congruent with {MK} methods, with most adaptive mutations occurring at the surface of proteins in both methods~\cite{Moutinho2019}.
These results still need to be refined across clades and genes, and the availability of polymorphism and divergence {DNA} sequences that are aligned will make this comparison possible.

However, positively selected genes or sites detected by classical {codon} models and {MK} methods can theoretically be different, since the non-adaptive part of substitutions  ($\omega_{\text{NA}}$) is subtracted in the {MK} test while classical {codon} models do not account for it.
...
As a result, mechanistic mutation-selection {codon} models ($\omega_0$) and the {MK} test ($\pnps$) should theoretically be more directly comparable.
From the availability of divergence and polymorphism data, it is now possible to ask whether the rate of non-adaptive evolution measured by phylogenetic mutation-selection models and {MK} methods are congruent, and if not the reason for such discrepancy should be understood.
...
Moreover, substitutions  are not instantaneous and ancestral polymorphisms can result in shared polymorphisms across species due to incomplete lineage sorting~\cite{Charlesworth2010}.

Mistaking polymorphisms for substitutions  is problematic, since both are not sensitive to mutation, selection and drift to the same extent~\cite{Mugal2014}.
...
In other words, phylogenetic and population-genetic approaches could be unified, in the context of a single modelling framework~\cite{Thorne2012}.

Such an integration between phylogenetic and population genetics has already been attempted in several studies.
For example, \cite{Wilson2011} modelled {codon} evolution in a joint framework with $3$ species, which allowed them to analyse the variation in selection pressure spatially along the genome and temporally between lineages.
...
However, it was found to be computationally intensive, even with the use of sufficient statistics to accelerate the computation.
Moreover, the assumption of a constant $\Ne$ along the phylogeny in mutation-selection {codon} models was arguably the strongest restriction to relax in this context~\cite{Rousselle2018}.
...
This was historically the reason why I decided to first extend phylogenetic site-specific mutation-selection {codon} models by incorporating branch-specific $\Ne$.
...
As it turns out, however, there are other issues that needed to be addressed, before achieving this integration, in particular, the fact that the range of $\Ne$ inferred by the model turns out to be too narrow, as well as computational issues.
