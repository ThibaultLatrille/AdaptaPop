...
\begin{document}
    ...
    Genes are classified under an adaptive regime if the lower bound for the posterior credible interval of gene-specific $\omega$ ($\alpha=0.05$) is above the upper bound of the posterior credible interval of gene-specific $\omega_{0}$.
    Because this is a unilateral test ($\omega > \omega_{0}$) and the two posterior credible interval are independent, the risk is $(\alpha/2)^2=0.025^2=6.25 \times 10^{-4}$ for each gene.
    ...
    For each gene, we computed the proportion of sites classified under an adaptive regime such that the lower bound for the posterior credible interval of site-specific $\omega$ ($\alpha=0.05$) is above 1.
