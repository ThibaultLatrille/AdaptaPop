...
\usepackage{biorxiv}
%DIF 28c28
%DIF < \usepackage[backend=biber,isbn=false,url=false,intitle=true,style=nature]{biblatex}
%DIF -------
...
\begin{document}
    ...
    This simple argument is not strictly valid in the presence of moderately deleterious non-synonymous mutations, which can segregate at substantial frequency in the population without reaching fixation, thus contributing solely to polymorphism, and not to divergence, potentially resulting on an underestimation of the rate of adaptive evolution\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{eyre-walker_quantifying_2002}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{mcdonald_adaptative_1991, eyre-walker_quantifying_2002}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
    ...
    \DIFdelbegin \DIFdel{Assuming synonymous mutations are neutral, $\omega>1$ signals }\DIFdelend \DIFaddbegin \DIFadd{Since this ratio is a contrast between the rates before and after the action of selection on the protein, it thus provides an estimate of the strength of selection exerted at the amino-acid level, whether synonymous changes are driven solely by mutation or by both mutation and selection\mbox{%DIFAUXCMD
\cite{yang_mutationselection_2008}}\hspace{0pt}%DIFAUXCMD
.
    In this context, }\DIFaddend an excess in \DIFdelbegin \DIFdel{the rate of }\DIFdelend non-synonymous substitutions\DIFdelbegin \DIFdel{compared to the neutral expectation, indicating }\DIFdelend \DIFaddbegin \DIFadd{, leading to $\omega>1$, indicates that the protein is undergoing recurrent positive selection for amino-acids changes, meaning }\DIFaddend that the protein is \DIFaddbegin \DIFadd{putatively }\DIFaddend under adaptive evolution.
    ...
    \DIFdelbegin \DIFdel{Accordingly, genes, or sites, }\DIFdelend \DIFaddbegin \DIFadd{For such comparison to be valid, both models estimating $\omega$ and $\omega_{0}$ should have assumptions as similar as possible regarding the underlying mutation process, modeled as a Muse \& Gaut\mbox{%DIFAUXCMD
\cite{muse_likelihood_1994, spielman_relationship_2015}}\hspace{0pt}%DIFAUXCMD
.
    ...
    \DIFadd{Given these validations, an excess of $\omega$ compared to $\omega_{0}$ can be retrieved as a ratio ($\omega / \omega_{0} > 1$) or as a difference ($\omega - \omega_{0}>0$).
    ...
    Altogether, genes or sites }\DIFaddend were considered to be under an adaptive regime (in red) if the value of their $\omega$ is higher than that of their $\omega_{0}$, with non-overlapping $95\%$ posterior credibility intervals.
    ...
    \DIFdelbegin \DIFdel{Of note, the 5 }\DIFdelend \DIFaddbegin \DIFadd{In particular, the five }\DIFaddend populations for which the test is not significant are the human populations.
    \DIFaddbegin \DIFadd{Coincidentally, this is the species with the lowest genetic diversity, resulting in under-powered statistical tests due to less SNPs available per loci.
    ...
    \DIFaddbegin \DIFadd{Moreover, because $\omega_{0}$ is by definition lower than 1\mbox{%DIFAUXCMD
\cite{spielman_relationship_2015}}\hspace{0pt}%DIFAUXCMD
...
\cite{kosiol_patterns_2008}}\hspace{0pt}%DIFAUXCMD
    ...
    Genes pertaining to these ontologies might be under adaptation as a consequence of being }\DIFaddend the target of \DIFdelbegin \DIFdel{virus and parasites}\DIFdelend \DIFaddbegin \DIFadd{viruses and parasites\mbox{%DIFAUXCMD
\cite{enard_viruses_2016, ebel_high_2017}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
.
    However, a strong CUB can influence the comparison between $\omega$ and $\omega_0$\mbox{%DIFAUXCMD
\cite{spielman_relationship_2015}}\hspace{0pt}%DIFAUXCMD
.
    In our mutation-selection codon model, in addition to model selection for amino-acids at the site level, modeling CUB is possible by introducing another layer of selection for synonymous codons at the gene level\mbox{%DIFAUXCMD
\cite{yang_mutationselection_2008, rodrigue_mechanistic_2010}}\hspace{0pt}%DIFAUXCMD
.
    ...
    Consequently, our estimation of the predicted rate of evolution computed at mutation-selection balance ($\omega_0$) is over-estimated given that epistasis is not taken into account \DIFdelbegin \DIFdel{, such that $\rateAphy = \omega - \omega_{0}$ is thus under-estimated.
    Altogether, we argue that our estimate of $\rateAphy$ is conservative and could be increased by modeling epistasis (altough indirectly) within the }\DIFdelend \DIFaddbegin \DIFadd{in our model.
    ...
    Accurately modeling the underlying fitness landscape is thus a direction for future research in }\DIFaddend mutation-selection \DIFdelbegin \DIFdel{framework\mbox{%DIFAUXCMD
\cite{goldstein_sequence_2017} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{models to improve the estimation of $\rateAphy$, by means of relaxing model assumptions\mbox{%DIFAUXCMD
\cite{goldstein_sequence_2017} }\hspace{0pt}%DIFAUXCMD
or by leveraging empirically determined fitness landscapes\mbox{%DIFAUXCMD
\cite{bloom_identification_2017, schrempf_scalable_2020}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
    ...
    Additionally, sequences from the species for which polymorphism are available, as well as their sister species have been discarded from the analysis to ensure independence between the data used in the phylogenetic and population-genetic method\DIFaddbegin \DIFadd{, such as to avoid fallacy and circularity in the estimation of $\rateApop$ and $\rateAphy$}\DIFaddend .
...
%DIFDELCMD <             \includegraphics[width=\linewidth, page=1]{scatterplot-gene-MutSel-0.025}
%DIFDELCMD <         \end{minipage}
%DIFDELCMD <         \llap{\raisebox{1.15cm}{\scriptsize A\hspace{4.55cm}}}\hfill
%DIFDELCMD <         \begin{minipage}{0.32\linewidth}
%DIFDELCMD <             \includegraphics[width=\linewidth, page=1]{scatterplot-site-MutSel-0.025}
%DIFDELCMD <         \end{minipage}
%DIFDELCMD <         \llap{\raisebox{1.15cm}{\scriptsize B\hspace{4.55cm}}}\hfill
%DIFDELCMD <         \begin{minipage}{0.32\linewidth}
%DIFDELCMD <             \includegraphics[width=\linewidth, page=1]{scatterplot-site-MutSelExclu-0.025}
%DIFDELCMD <         \end{minipage}
%DIFDELCMD <         \llap{\raisebox{1.15cm}{\scriptsize C\hspace{4.55cm}}}\hfill
%DIFDELCMD <         %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\linewidth, page=1]{figure1}
        \DIFaddendFL \caption{
